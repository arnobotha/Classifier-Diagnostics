...
# - Fit model
logitMod_Bas <- glm(inputs_bas, data=datCredit_train, family="binomial")


# --- 4.2 Assessment
# - Deviance and AIC
summary(logitMod_Bas)
### RESULTS: Null deviance = 279124; Residual deviance = 262800; AIC = 262814

# - Evaluate fit using generic R^2 based on deviance vs null deviance
coefDeter_glm(logitMod_Bas)
### RESULTS: 5.85%

# - Odds Ratio analysis
round(exp(cbind(OR = coef(logitMod_Bas), confint.default(logitMod_Bas))), 3)
### RESULTS: odds ratios of [Term], [Principal_Real], and [Balance_Real] are all close to 1, which limits their usefulness (however, the range of these variables may be influencing this result; a standardised odds ratio may be more insightful)

# - Residual deviance analysis
resid_deviance_glm(logitMod_Bas)
### RESULTS: Model fit is strained (3 diagnostics gave warnings)

# - Variable importance
varImport_logit(logitMod_Bas, method="stdCoef_Goodman", sig_level=0.1, impPlot=T, plotVersionName="Basic")
### RESULTS: Top three variables: [PerfSpell_Num], [Balance_Real], and [Principal_Real]

# - ROC analysis
datCredit_train[, prob_bas := predict(logitMod_Bas, newdata = datCredit_train, type="response")]
datCredit_valid[, prob_bas := predict(logitMod_Bas, newdata = datCredit_valid, type="response")]
