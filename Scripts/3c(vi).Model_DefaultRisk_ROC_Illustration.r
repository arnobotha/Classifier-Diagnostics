...
###           The model is not overfitted as evidenced by the AUCs computed on the training- and validation datasets being close to one another (79.49% vs 79.26%)
...
logitMod_2 <- glm(DefaultStatus1_lead_12_max ~ PerfSpell_Num + g0_Delinq + g0_Delinq_SD_4 + g0_Delinq_SD_6
                  , data=datCredit_train, family="binomial")
summary(logitMod_2) # Null deviance = 255631; Residual deviance = 187524; AIC = 187540
# - FIRM analysis
...
### RESULTS:  All estimated coefficients and their associated standard errors seem reasonable
###           The model is not overfitted as evidenced by the AUCs computed on the training- and validation datasets being close to one another (83.12% vs 82.73%)
...
logitMod_3 <- glm(DefaultStatus1_lead_12_max ~ TimeInPerfSpell + Balance + Principal + InterestRate_Margin_imputed_mean + # Core variables
                    ...
                    g0_Delinq_Any_Aggr_Prop + BookMaturity_Aggr_Mean + NewLoans_Aggr_Prop_1 # Portfolio level variables
                  , data=datCredit_train, family="binomial")
...
### RESULTS:  All estimated coefficients and their associated standard errors seem reasonable
###           The model is not overfitted as evidenced by the AUCs computed on the training- and validation datasets being close to one another (86.88% vs 87.83%)
...
dat_anno <- data.table(Model=c("a_Basic","b_Semi_advanced","c_Advanced"),
...
(g_ROC_compar <- ggplot(datPlot) + theme_minimal() +
  ...
  geom_abline(intercept=0, slope=1, linetype=2, linewidth=0.3) +
...
rm(auc_logitMod_1, auc_logitMod_2, auc_logitMod_3,
   roc_obj_1, roc_obj_2, roc_obj_3,
